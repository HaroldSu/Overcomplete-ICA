...
output_k <- sapply(seq(20, 100, by = 5), function(k){
  output <- sapply(1:nrep, function(replicate){
    ...
    Hs <- CU[,1:k]
    ## The ds estimates
    t1 <- system.time({ds_est_oica_semiada <- sdp_semiada(Hs, k)})
    t2 <- system.time({ds_est_fpca <- fourier_pca(X, k)})
    ## A-Error
    ...
    ## Runtime
    runtime1 <- as.numeric(t1)
    runtime1 <- runtime1[2]
    runtime2 <- as.numeric(t2)
    runtime2 <- runtime1[2]
    ## Perfect Recovery
    ...
    return(c(a_error_oica_semiada, a_error_fpca,
             f_error_oica_semiada, f_error_fpca,
             runtime1, runtime2))
...
colnames(output_k) <- c("a_error_oica_semiada", "a_error_fpca",
                        "f_error_oica_semiada", "f_error_fpca",
                        "runtime_oica_semiada", "runtime_fpca")
# "perf_recovery_oica_semiada", "perf_recovery_rand")
output_k <- as.data.frame(output_k)

## A-Error
plot(y = output_k$a_error_oica_semiada, x = seq(20, 100, by = 5),
     col = "red", type = "b",
     xlab = "latent dimension", ylab = "A-Error", ylim = c(0,1))
points(y = output_k$a_error_rand, x = seq(20, 100, by = 5),
       col = "orange", type = "b")

## F-Error
plot(y = output_k$f_error_oica_semiada, x = seq(20, 100, by = 5),
     col = "red", type = "b",
     xlab = "latent dimension", ylab = "F-Error", ylim = c(0,1))
points(y = output_k$f_error_rand, x = seq(20, 100, by = 5),
       col = "orange", type = "b")

## Runtime
plot(y = log(output_k$runtime_oica_semiada), x = seq(20, 100, by = 5),
     col = "red", type = "b",
     xlab = "latent dimension", ylab = "Runtime (log-sclae)", ylim = c(0,1))
points(y = log(output_k$runtime_fpca), x = seq(20, 100, by = 5),
       col = "blue", type = "b")
